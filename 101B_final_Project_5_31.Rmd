...
### based on the result above, make a reduced model
model.reduced <- lm(CV ~ classwt + cutoff + maxnodes + classwt:maxnodes + cutoff:maxnodes +
      classwt:cutoff + replace:nodesize, data = coded.frac.design
  )
...
X.frac <- model.matrix(~(classwt + cutoff + maxnodes + classwt:maxnodes + cutoff:maxnodes +
      classwt:cutoff + replace:nodesize)^2-1, data.frame(new_data)) # matrix with main effects and
...
#### VIF shows that cutoff:maxnodes seems to be problematic

### So, get rid of cutoff:maxnodes from the model and make a new.model.reduced. And then create q-q plot and residual plots.
new.model.reduced <- lm(CV ~ classwt + cutoff + maxnodes + classwt:maxnodes +
      classwt:cutoff + replace:nodesize, data = coded.frac.design
  )
...
new.X.frac <- model.matrix(~(classwt + cutoff + maxnodes + classwt:maxnodes  +
      classwt:cutoff + replace:nodesize)^2-1, data.frame(new_data)) # matrix with main effects and
var.eff.one <- diag(solve(t(new.X.frac)%*%new.X.frac))

results.frac <- data.frame('Var.32run' = var.eff.one, 'VIF.32run' = nrow(new.X.frac)*var.eff.one)
print.data.frame(results.frac)

### As a result from new.reduced.model, VIF gives even higher number which is 9.31
...
```{r}
orig.model <- lm(CV~., data = new.opt.design)
summary(orig.model)

coded.model <- lm(CV~., data = extraction.data.coded)
summary(coded.model) # Here, we now want to find significant effects

coded.model.reduced <- lm(CV~ntree+mtry+classwt+maxnodes, data = extraction.data.coded)
summary(coded.model.reduced)


# q-q and residual plots
yield.resid <- residuals(coded.model.reduced) 
pred.yield <- fitted(coded.model.reduced)
par(mfrow = c(1,3))
qqnorm(yield.resid); qqline(yield.resid)
plot(x = pred.yield, y = yield.resid, 
     xlab = "Predicted", ylab = "Residuals")
...
model1 <- lm(CV ~ ntree * mtry * replace * nodesize * classwt * cutoff * maxnodes, data = new.opt.design) # model with 2-interactions
```
